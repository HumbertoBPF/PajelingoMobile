...
public class MeaningSynchro extends ResourcesSynchro<Meaning> {
    public MeaningSynchro(Context context, NotificationCompat.Builder builder) {
        super(context, builder, 50, AppDatabase.getInstance(context).getMeaningDao(),
                () -> LanguageSchoolAPIHelper.getApiObject().getMeanings(), new WordSynchro(context, builder));
    }
}
