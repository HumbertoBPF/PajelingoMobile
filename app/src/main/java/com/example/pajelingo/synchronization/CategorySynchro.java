...
public class CategorySynchro extends ResourcesSynchro<Category> {
    public CategorySynchro(Context context, NotificationCompat.Builder builder, OnTaskListener onTaskListener) {
        super(context, builder, 13, AppDatabase.getInstance(context).getCategoryDao(),
                () -> LanguageSchoolAPIHelper.getApiObject().getCategories(),
                new ConjugationSynchro(context, builder, onTaskListener), onTaskListener);
    }
}
