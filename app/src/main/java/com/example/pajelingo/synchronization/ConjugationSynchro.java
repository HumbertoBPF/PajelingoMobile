...
public class ConjugationSynchro extends ResourcesSynchro<Conjugation> {
    public ConjugationSynchro(Context context, NotificationCompat.Builder builder, OnTaskListener onTaskListener) {
        super(context, builder, 25, AppDatabase.getInstance(context).getConjugationDao(),
                () -> LanguageSchoolAPIHelper.getApiObject().getConjugations(),
                new LanguageSynchro(context, builder, onTaskListener), onTaskListener);
    }
}
