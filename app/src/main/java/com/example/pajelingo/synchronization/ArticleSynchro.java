...
public class ArticleSynchro extends ResourcesSynchro<Article>{
    public ArticleSynchro(Context context, NotificationCompat.Builder builder, OnTaskListener onTaskListener) {
        super(context, builder, 0, AppDatabase.getInstance(context).getArticleDao(),
                () -> LanguageSchoolAPIHelper.getApiObject().getArticles(),
                new CategorySynchro(context, builder, onTaskListener), onTaskListener);
    }
}
